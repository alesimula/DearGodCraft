...
public final class ConcurrentIdentityHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
    ...
    static final class Segment<K, V> extends ReentrantLock {
        ...
        private void removeEntry(int index, int c, HashEntry<K, V>[] tab, HashEntry<K, V> first, HashEntry<K, V> e) {
            ++ modCount;
        ...
        V compute(K key, int hash, BiFunction<? super K, ? super V, ? extends V> remappingFunction, Boolean onlyIfAbsent) {
            lock();
            try {
                V newValue = null;
                int c = count;
                if (onlyIfAbsent != null && c ++ > threshold) { // ensure capacity
                    int reduced = rehash();
                    if (reduced > 0) {
                        count = (c -= reduced) - 1; // write-volatile
                    }
                ...
                if (e != null) {
                    oldValue = e.value();
                    if ((onlyIfAbsent == null || !onlyIfAbsent) && oldValue != null) {
                        newValue = remappingFunction.apply(key, oldValue);
                        if (newValue != oldValue && newValue != null) e.setValue(newValue);
                        else if (newValue == null) removeEntry(index, c, tab, first, e);
                    }
                    else if (oldValue == null && onlyIfAbsent != null && onlyIfAbsent) {
                        newValue = remappingFunction.apply(key, null);
                        if (newValue != null) e.setValue(newValue = remappingFunction.apply(key, null));
                    }
                } else if (onlyIfAbsent != null && (newValue = remappingFunction.apply(key, null)) != null) {
                    ++ modCount;
                    tab[index] = newHashEntry(key, hash, first, newValue);
                    count = c; // write-volatile
                }
    ...
    public V compute(K key, BiFunction<? super K, ? super V, ? extends V> remappingFunction) {
        int hash = hashOf(key);
        return segmentFor(hash).compute(key, hash, remappingFunction, false);
    }

    @Override
    public V computeIfAbsent(K key, Function<? super K, ? extends V> mappingFunction) {
        int hash = hashOf(key);
        return segmentFor(hash).compute(key, hash, (k,v)->mappingFunction.apply(k), true);
    }
